 ...
 * This test adds multilpe files and directory in the source directory
 * and expect that file to be copied into destination
 * directory by dropbox application.
 */
class FilesAndDirAddition: public ::testing::Test
{
public:
    /// Class constructor.
    FilesAndDirAddition()
    ...
    void SetUp()
    {
        create_dir(dst_dir);
        system("pwd");
        system("cp -r ../../test_integration/data/test_srcDir .");
    }
    ...
    void server_process()
    {
        DropboxServer server(dst_dir, "127.0.0.1", 8050, 20);
    ...
    void client_process()
    {
        DropboxClient client(src_dir, "127.0.0.1", 8050, 20);
        client.start();
    }
...
TEST_F (FilesAndDirAddition, FilesAndDir)
{
    // Start the dropbox server instance
    try
    {
        server_thread = std::thread(std::bind(&FilesAndDirAddition::server_process, this));
    }
    ...
    try
    {
        client_thread = std::thread(std::bind(&FilesAndDirAddition::client_process, this));
    }
    catch(const std::exception& e)
    {
        std::cerr << e.what() << std::endl;
